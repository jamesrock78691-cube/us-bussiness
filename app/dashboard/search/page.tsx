...
function phoneSearchUrl(b: Business) {
  const q = `"${b.companyName}" ${b.city || ""} ${b.state} (phone OR tel OR "call us")`;
  return `https://www.google.com/search?q=${encodeURIComponent(q)}`;
}

function emailSearchUrl(b: Business) {
  const q = `"${b.companyName}" ${b.city || ""} ${b.state} (email OR contact@ OR info@)`;
  return `https://www.google.com/search?q=${encodeURIComponent(q)}`;
}
...
export default function SearchPage() {
  ...
  async function handleExportCSV() {
    ...
    try {
      ...
      setExportProgress(`Done — ${unique.length} rows`);
    } catch {
  ...
  const totalPages =
    result
      ? Math.max(result.totalPages || Math.ceil((result.total || 0) / 20) || 1, 1)
      : 1;
  const showPager = Boolean(result && (totalPages > 1 || (result.total || 0) > 20));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input type="text" value={filters.zip} onChange={(e) => setFilters({ ...filters, zip: e.target.value })} onKeyDown={handleKeyDown} placeholder="ZIP" className="w-full px-3 py-2.5 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
          </div>
          <div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <span className="text-sm font-medium text-slate-700">Results</span>
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-xs text-slate-400">{result ? `${result.total.toLocaleString()} total` : "—"}</span>
            {showPager && (
              <div className="flex items-center gap-1">
                <button type="button" disabled={page <= 1 || loading} onClick={() => setPage((p) => Math.max(1, p - 1))} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 disabled:opacity-40">
                  <ChevronLeft className="w-4 h-4" /> Prev
                </button>
                <span className="text-xs text-slate-700 px-2 font-semibold">Page {page} / {totalPages.toLocaleString()}</span>
                <button type="button" disabled={page >= totalPages || loading} onClick={() => setPage((p) => p + 1)} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 disabled:opacity-40">
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[1400px]">
                <thead>
                  <tr className="bg-slate-50 text-left text-[10px] font-semibold text-slate-500 uppercase">
                  ...
                  {result.data.map((b) => {
                    ...
                    return (
                      <tr key={b.id} className="hover:bg-slate-50/80">
                        <td className="px-3 py-2 font-medium text-slate-900 max-w-[180px] sticky left-0 bg-white">
                        ...
                        <td className="px-3 py-2 text-xs">
                          <a href={mapsUrl(b)} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline inline-flex items-center gap-0.5">Maps <ExternalLink className="w-3 h-3" /></a>
                        </td>
                        <td className="px-3 py-2 text-[10px] whitespace-nowrap">
                          <span className="text-slate-600 mr-1">{cell(tm)}</span>
                          <a href={usptoUrl(b.companyName)} target="_blank" rel="noopener noreferrer" onClick={() => markTrademark(b.id, "Checked")} className="text-violet-700 hover:underline mr-1">USPTO</a>
